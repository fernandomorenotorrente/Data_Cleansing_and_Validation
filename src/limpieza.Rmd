...
datosAdult <- read.csv('data_cleaned.csv',stringsAsFactors = FALSE, header = FALSE)
...
Una vez analizados los valores outliers a través de los bloxplot procedemos a decidir que haremos para tratarlos.

En primer lugar para la variable Age, ya que tenemos extremos que en principio no pertenecen al conjunto de la población activa, eliminaremos los valores mas extremales, sobre todo los que corresponden al rango mas alto de edades. Es plausible que una persona de 17 años trabaje, pero sin embargo, no lo es que una de 90 años lo haga. 

De esta forma eliminaremos los registros con valores de Age mayores a 65 años.

```{r echo=TRUE, message=FALSE, warning=FALSE, fig.show='hold', fig.align = "center", fig.width=30, fig.height=20}
datosAdult<- datosAdult[datosAdult$Age <= 65]
```

En cuanto a los valores outliers de la variable education, parecen perfectamente plausibles, es decir, tenemos que abarcar casuísticas en las que la formación de los ciudadanos sea pobre o por contraparte elevada.

Ya que la variable fnlwgt corresponde a pesos sociodemográficos, eliminar o tratar de forma distinta valores extremales sería modificar los casos de estudio mas representativos dentro de nuestra investigación, por lo que deberemos 
...
Dado que estos atributos nos aportan muy poca información, eliminaremos estas columnas. La amplia mayoría tienen el mismo valor, por lo cuál no será significativa esta información en nuestro estudio.

```{r echo=TRUE, message=FALSE, warning=FALSE, fig.show='hold', fig.align = "center", fig.width=30, fig.height=20}
#nos quedamos aqui con el mismo dataset pero sin estas columnas
...
En este contexto, estableceremos ahora los grupos en los que vamos a organizar el estudio. 

En primer lugar agruparemos las casuísticas en sexos. Esto puede ser bastante interesante ya que puede darnos pruebas bajo estudio de la brecha salarial entre sexos.

```{r echo=TRUE, message=FALSE, warning=FALSE, fig.show='hold', fig.align = "center", fig.width=30, fig.height=20}
datosAdult.women <- datosAdult[datosAdult$sex='Female']

datosAdult.men <- datosAdult[datosAdult$sex='Male']

```

Otra agrupación interesante sería analizar los rangos de edad de la población para eso haremos distintos cinco grupos distintos, los comprendidos entre 17 y 25 años, de 25 a 35, de 45 a 65.

```{r echo=TRUE, message=FALSE, warning=FALSE, fig.show='hold', fig.align = "center", fig.width=30, fig.height=20}

datosAdult.junior <- datosAdult[datosAdult$Age>=17 && datosAdult$Age<=25]

datosAdult.bigjunior <- datosAdult[datosAdult$Age>25 && datosAdult$Age<=35]

datosAdult.senior <- datosAdult[datosAdult$Age>35 && datosAdult$Age<=45]

datosAdult.bigsenior <- datosAdult[datosAdult$Age>45 && datosAdult$Age<=65]
