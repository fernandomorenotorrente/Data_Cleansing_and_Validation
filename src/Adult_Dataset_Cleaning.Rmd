...
**workclass**: **Private 68%**, Self-emp-not-inc 8%, Self-emp-inc 3%, Federal-gov 3%, Local-gov 6%, State-gov 4%, Without-pay 1.5%, Never-worked 0.5%, **Valor desconocido ("?") 5%**.    
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
count(filter(datosAdult, occupation=="?" & workclass=="?"))
```
...
Pero no ocurre lo mismo para native.country:   
```{r echo=TRUE, message=FALSE, warning=FALSE}
count(filter(datosAdult, occupation=="?" & native.country=="?"))
```
...
En los atributos tipo factor nos llaman la atención el atributo de "native.country" (con "United-States": 90%) y el atributo de "race" (con "White": 85%).
...
Vamos a crear boxplots para analizar los atributos numéricos.
...
grid.arrange(ggboxplot(datosAdult, x ="income" , y = "age", color = "income"),
          ...
          ggboxplot(datosAdult, x ="income" , y = "hours.week", color = "income"), ncol = 2)
